...
export default function ChatPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const botReply = data.choices?.[0]?.message?.content || "I'm sorry, I didn't understand that."
      setMessages((prev) => [...prev, { role: "assistant", text: botReply }])
    } catch (err) {
      console.error("Error:", err.message)
      setMessages((prev) => [...prev, { role: "assistant", text: "Error contacting OpenRouter API." }])
    } finally {
      setLoading(false)
    }
  }
  ...
  <div className="pt-16 min-h-screen flex flex-col bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white">
    <div className="flex-1 overflow-y-auto px-4 pb-36 pt-6">
      ...
      {loading && (
        <div className="text-sm italic text-gray-500 dark:text-gray-400">Bot is typing...</div>
      )}
      <div ref={endRef} />
    </div>
